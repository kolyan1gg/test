...
from short_model import get_categories_rn, get_categories_vit, save_result_as_chart
...
# Animal list
animal_list = ['Bear', 'Brown bear', 'Bull', 'Camel', 'Canary', 'Cat', 'Caterpillar', 'Cattle', 
    'Centipede', 'Cheetah', 'Chicken', 'Crab', 'Crocodile', 'Deer', 'Dog', 'Duck', 
    'Eagle', 'Elephant', 'Fish', 'Fox', 'Frog', 'Giraffe', 'Goat', 'Goldfish', 'Goose', 
    'Hamster', 'Harbor seal', 'Hedgehog', 'Hippopotamus', 'Horse', 'Jaguar', 'Jellyfish', 
    'Kangaroo', 'Koala', 'Ladybug', 'Leopard', 'Lion', 'Lizard', 'Lynx', 'Magpie', 
    'Monkey', 'Moths and butterflies', 'Mouse', 'Mule', 'Ostrich', 'Otter', 'Owl', 'Panda', 
...
async def cmd_start(message: types.Message):
    await message.answer(f'Привет! Я определяю зверушку по фотографии. Отправь мне фотографию, и я скажу кто на ней изображен. \nНапиши /help, и я покажу пример, как я работаю')
    await message.answer(f'Для справки список всех команд: \n/help \n/info \n/example \n/help \n/g [запрос] \n/g')
    await message.answer(f'Если лень искать картинку, можешь написать /g [запрос] - например, "/g злой тигр", я найду 20 картинок по твоему запросу в Google Images, выберу случайную и определю её класс')
...
async def cmd_help(message: types.Message):
    ...
    await message.answer(f'Выбираем модель и нажимаем кнопку "Подтвердить"')
...
async def callbacks_num(callback: types.CallbackQuery):
    ...
    if action == "RESNET":
        ...
        if user_value == "RESNET":
            ...
            await callback.message.answer('Обратная связь', reply_markup=builder.as_markup())
